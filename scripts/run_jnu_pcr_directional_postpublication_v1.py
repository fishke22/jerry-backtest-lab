...
DEFAULT_PREREG = ROOT / "config" / "jnu_pcr_directional_postpublication_prereg_v1.json"
NIKKEI_FUTURES_CSV = "https://indexes.nikkei.co.jp/nkave/historical/nikkei_225_futures_index_series_daily_en.csv"
...
def fetch_futures_index() -> tuple[dict[str, list[tuple[date, float]]], str]:
    req = urllib.request.Request(NIKKEI_FUTURES_CSV, headers={"User-Agent": "JerryBacktestLab/PCR-Replication-1.0"})
    ...
    for row in rows[1:]:
        if len(row) < 2:
            continue
        try:
            d = date.fromisoformat(row[0].strip().replace("/", "-"))
            v = float(row[1].replace(",", "").strip())
        except Exception:
            continue
...
def main() -> None:
    ...
    ap.add_argument("--output", type=Path, default=ROOT / "results" / "jnu_pcr_directional_postpublication_v1.json")
    ap.add_argument("--report", type=Path, default=ROOT / "reports" / "jnu_pcr_directional_postpublication_v1.md")
    args = ap.parse_args()
    ...
    result = {
        ...
        "source": {
            ...
            "nikkei_futures_index_url": NIKKEI_FUTURES_CSV,
    ...
    lines = [
        "# JNU PCR post-publication directional replication v1",
        "",
        f"- Status: **{status}**",
