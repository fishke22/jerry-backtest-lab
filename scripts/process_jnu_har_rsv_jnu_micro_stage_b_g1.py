...
PREREG = ROOT / "config" / "jnu_har_rsv_jnu_micro_stage_b_g1_prereg.json"
...
RESULT = ROOT / "volatility_results" / "jnu_har_rsv_jnu_micro_stage_b_g1.json"
REPORT = ROOT / "volatility_reports" / "jnu_har_rsv_jnu_micro_stage_b_g1.md"
...
def load_and_gate() -> tuple[pd.DataFrame, dict, dict]:
    ...
    if stage_a.get("status") != "TRUE_OSE_MINI_STAGE_A_PASS" or stage_a.get("stage_a_pass") is not True:
...
def main() -> None:
    ...
    stage_b_pass = bool(beats_both and bootstrap_ok)

    status = "TRUE_JNU_MICRO_STAGE_B_PASS" if stage_b_pass else "TRUE_JNU_MICRO_STAGE_B_FAIL"
    result = {
        "candidate_id": prereg["candidate_id"],
        "status": status,
        "stage_b_pass": stage_b_pass,
        "stage_a_prerequisite": "TRUE_OSE_MINI_STAGE_A_PASS",
        "promotion_pipeline_stage": 3,
        "promotion_state_if_pass": "INFORMATION_STATE_CANDIDATE",
        "role": "VOLATILITY_RISK_STATE_ONLY_NOT_DIRECTIONAL_ALPHA",
        "source": "exact-product OSE Nikkei 225 Micro (JNU) daily RV/RSV derived locally from 225Labo 5-minute data",
        ...
        "next_rule": (
            "Stage 3 true-target confirmation passes. HAR-RSV may advance only as an information/risk-state candidate; any position-sizing/stop/confidence rule must enter applicable later promotion stages."
            if stage_b_pass
            else "Do not admit HAR-RSV as a JNU-confirmed information module. Do not rescue by retuning Micro or reinterpreting Stage A."
        ),
    ...
    REPORT.write_text(
        "# JNU HAR-RSV Exact-Product Micro Stage B G1\n\n"
        f"- Status: **{status}**\n"
        ...
        "This is exact-product OSE Nikkei 225 Micro (JNU) Stage B confirmation of the frozen HAR-RSV family. "
        "A PASS is risk/information-state evidence only, not directional alpha and not Stage 9 validation.\n",
        encoding="utf-8",
    )
