...
FRAMEWORK=ROOT/"config"/"jnu_operational_framework_current_v1_2.json"
PREREG=ROOT/"config"/"jnu_operational_live_shadow_prereg_v1_1.json"
IMPL=ROOT/"config"/"jnu_operational_live_shadow_implementation_v1_1.json"
...
def main():
    ...
    missing=[k for k in impl["required_fields"] if k not in x]
    if missing:
    ...
    if created.utcoffset()!=timedelta(hours=8):
        raise RuntimeError("created_at_taipei must have +08:00 offset")
    if ref>created:
        raise RuntimeError("reference timestamp is after forecast creation")
    age=(created-ref).total_seconds()/60
    if age<0 or age>15:
        raise RuntimeError(f"reference age {age:.2f} minutes exceeds 15")
    if x.get("exact_product") is not True:
        raise RuntimeError("exact_product must be true")
    if float(x["reference_price"])<=0:
        raise RuntimeError("reference_price must be positive")
    if x["bias"] not in {"BULLISH","BEARISH","NEUTRAL_ABSTAIN"}:
        raise RuntimeError("invalid bias")
    if x["confidence"] not in {"LOW","MEDIUM"}:
    ...
    for k in ["reference_source","expected_path","invalidation_conditions","event_risk","flip_conditions","evidence_summary"]:
        if not str(x[k]).strip():
            raise RuntimeError(f"{k} must be nonempty")
    trace=x["decision_trace"]
    if not isinstance(trace,dict):
        raise RuntimeError("decision_trace must be an object")
    protocol_sha=sha(PROTOCOL)
    if trace.get("protocol_sha256")!=protocol_sha:
        raise RuntimeError("decision_trace protocol SHA mismatch")
    if trace.get("bias")!=x["bias"]:
        raise RuntimeError("forecast bias does not match decision_trace bias")
    if trace.get("confidence")!=x["confidence"]:
        raise RuntimeError("forecast confidence does not match decision_trace confidence")
    if trace.get("calibrated_probability") is not False:
        raise RuntimeError("decision_trace calibrated_probability must be false")
    ...
    if out.exists():
        raise RuntimeError(f"forecast already exists: {out}")
    out.write_text(json.dumps(base,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    print(json.dumps({"status":"FORECAST_REGISTERED_IMMUTABLE_V1_1","forecast_id":fid,"path":str(out),"record_sha256":sha(out)},ensure_ascii=False,indent=2))
