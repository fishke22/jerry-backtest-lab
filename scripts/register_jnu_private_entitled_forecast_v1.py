...
def main():
    ...
    try:
        wr=write_immutable_json(a.private_ledger_root,out,record)
    except FileExistsError:
        if verify_backup(a.private_ledger_root,out).get("status")!="PASS":raise RuntimeError("concurrent private forecast exists but backup integrity invalid")
        old=load(out)
        if old.get("request_record_sha256")==request_sha and old.get("private_quote_receipt_id")==bundle.get("receipt_id"):
            print(json.dumps({"status":"PRIVATE_ENTITLED_FORECAST_ALREADY_REGISTERED","forecast_id":fid,"raw_and_derived_fields_not_printed":True,"public_output_created":False,"real_public_ledger_modified":False},indent=2));return
        raise RuntimeError("PRIVATE_FORECAST_IDEMPOTENCY_CONFLICT")
    status="PRIVATE_ENTITLED_FORECAST_ALREADY_REGISTERED" if wr.get("status")=="ALREADY_PRESENT" else "PRIVATE_ENTITLED_FORECAST_REGISTERED"
    print(json.dumps({"status":status,"forecast_id":fid,"raw_and_derived_fields_not_printed":True,"public_output_created":False,"real_public_ledger_modified":False},indent=2))
