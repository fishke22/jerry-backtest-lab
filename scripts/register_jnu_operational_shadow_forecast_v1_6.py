...
def main():
    ...
    req_created=parse_dt(str(x["request_created_at_taipei"]))
    req_until=parse_dt(str(x["request_valid_until_taipei"]))
    ...
    if request_window<=0 or request_window>900: raise RuntimeError("request validity window must be >0 and <=900 seconds")
    if created<req_created or created>req_until: raise RuntimeError("forecast creation is outside immutable request validity window")
    ...
    if str(meta.get("symbol","")).upper()!=str(x["symbol"]).upper():
        raise RuntimeError("forecast symbol does not match source metadata symbol")
    ...
    preparation_validation=validate_prepared_request_binding(
        x, decision_event_state=(trace.get("risk_modifiers") or {}).get("event_state")
    )
    risk_validation=validate_risk_state_evidence(
        x["risk_state_evidence"],trace,str(x["created_at_taipei"]),
        request_created_at=str(x.get("request_created_at_taipei")) if x.get("request_created_at_taipei") else None,
        target_day_session_date=str(x["target_day_session_date"]),
    )
    base={
      **x,
      "reference_source_class":source_class,
      "risk_state_evidence_validation":risk_validation,
      "request_preparation_validation":preparation_validation,
    ...
    print(json.dumps({"status":"FORECAST_REGISTERED_IMMUTABLE_V1_6","forecast_id":fid,"path":str(out),"record_sha256":sha(out)},ensure_ascii=False,indent=2))
