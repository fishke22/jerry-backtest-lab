...
from jnu_risk_state_evidence_validation_v1_1 import validate_risk_state_evidence
from jnu_request_preparation_validation_v1 import validate_prepared_request_binding
...
OUT=ROOT/"live_shadow"/"results"/"jnu_operational_live_shadow_current_v1_5.json"
PROTOCOL=ROOT/"config"/"jnu_operational_decision_protocol_v1.json"
FRAMEWORK=ROOT/"config"/"jnu_operational_framework_current_v1_7.json"
PREREG=ROOT/"config"/"jnu_operational_live_shadow_prereg_v1_6.json"
IMPL=ROOT/"config"/"jnu_operational_live_shadow_implementation_v1_6.json"
HASH_PROTOCOL=ROOT/"config"/"jnu_integrity_hash_protocol_v1.json"
REQUEST_PROTOCOL=ROOT/"config"/"jnu_cloud_forecast_request_protocol_v1_2.json"
PREP_PROTOCOL=ROOT/"config"/"jnu_cloud_request_preparation_protocol_v1.json"
...
def main():
    ...
    if args.forecast_dir.exists():
        for p in sorted(args.forecast_dir.glob("*.json")):
            ...
            if f.get("integrity_hash_method")!=METHOD_ID: raise RuntimeError(f"forecast {fid} integrity hash method mismatch")
            if f.get("integrity_hash_protocol_sha256")!=sha(HASH_PROTOCOL): raise RuntimeError(f"forecast {fid} integrity hash protocol SHA mismatch")
            if f.get("cloud_request_protocol_sha256")!=sha(REQUEST_PROTOCOL): raise RuntimeError(f"forecast {fid} cloud request protocol SHA mismatch")
            if f.get("request_preparation_protocol_sha256")!=sha(PREP_PROTOCOL): raise RuntimeError(f"forecast {fid} request preparation protocol SHA mismatch")
            if f.get("official_event_state_protocol_sha256")!=sha(EVENT_PROTOCOL): raise RuntimeError(f"forecast {fid} official event-state protocol SHA mismatch")
            created=dt(f["created_at_taipei"])
            req_created=dt(f["request_created_at_taipei"])
            req_until=dt(f["request_valid_until_taipei"])
            request_window=(req_until-req_created).total_seconds()
            if request_window<=0 or request_window>900:
                raise RuntimeError(f"forecast {fid} request validity window invalid")
            if created<req_created or created>req_until:
                raise RuntimeError(f"forecast {fid} created outside immutable request window")
            meta=f.get("reference_source_metadata")
            if str(meta.get("symbol","")).upper()!=str(f.get("symbol","")).upper():
                raise RuntimeError(f"forecast {fid} symbol/source metadata mismatch")
            source_class=validate_reference_source_metadata(meta,float(f.get("reference_price")),str(f.get("reference_timestamp")))
            if f.get("reference_source_class")!=source_class: raise RuntimeError(f"forecast {fid} reference source class mismatch")
            ...
            if real and integrity_forecasts==0 and (tr.get("risk_modifiers") or {}).get("event_state")=="UNKNOWN":
                raise RuntimeError(f"forecast {fid} first real registration has UNKNOWN event state")
            ...
            pv=validate_prepared_request_binding(
                f, decision_event_state=(tr.get("risk_modifiers") or {}).get("event_state")
            )
            if f.get("request_preparation_validation")!=pv:
                raise RuntimeError(f"forecast {fid} request-preparation validation mismatch")
            rv=validate_risk_state_evidence(
                f.get("risk_state_evidence"),tr,str(f.get("created_at_taipei")),
                request_created_at=str(f.get("request_created_at_taipei")) if f.get("request_created_at_taipei") else None,
                target_day_session_date=str(f.get("target_day_session_date")),
            )
            if f.get("risk_state_evidence_validation")!=rv:
                raise RuntimeError(f"forecast {fid} risk-state evidence validation mismatch")
    ...
    if args.outcome_dir.exists():
        for p in sorted(args.outcome_dir.glob("*.json")):
            ...
            if o.get("forecast_record_sha256_method")!=METHOD_ID:
                raise RuntimeError(f"outcome {fid} forecast hash method mismatch")
            if o.get("integrity_hash_protocol_sha256")!=sha(HASH_PROTOCOL):
                raise RuntimeError(f"outcome {fid} integrity hash protocol SHA mismatch")
    ...
    result={
      "version":"1.5","status":status,
      "integrity":{"status":"PASS","forecasts_verified":integrity_forecasts,"outcomes_verified":integrity_outcomes,
                   "git_tracking_enforced":real,"decision_trace_recomputed":True,
                   "risk_state_evidence_recomputed":True,"prepared_request_recomputed":True,"outcomes_recomputed":True,
                   "protocol_sha256":protocol_sha},
      "framework_sha256":sha(FRAMEWORK),"prereg_sha256":sha(PREREG),"implementation_sha256":sha(IMPL),
      "integrity_hash_method":METHOD_ID,"integrity_hash_protocol_sha256":sha(HASH_PROTOCOL),
      "cloud_request_protocol_sha256":sha(REQUEST_PROTOCOL),
      "request_preparation_protocol_sha256":sha(PREP_PROTOCOL),
      "official_event_state_protocol_sha256":sha(EVENT_PROTOCOL),
