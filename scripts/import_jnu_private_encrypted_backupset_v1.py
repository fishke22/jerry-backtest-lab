...
from validate_jnu_private_restore_authorization_v1 import authorization_use_guard,load as load_auth,mark_authorization_used,validate as validate_auth
...
def main():
    ...
    bs=a.backupset.resolve()
    require_external(bs,"private encrypted backupset")
    ...
    if bm.get("algorithm")!="AES-256-GCM" or bm.get("envelope") is not True or bm.get("key_material_present") is not False or bm.get("plaintext_hash_present") is not False:
    ...
    with authorization_use_guard(a.authorization,av["restore_id"]) as usage_receipt:
        if dest.exists() and any(dest.iterdir()):
            raise RuntimeError("encrypted DR import destination must be empty")

        _kmeta,kek=key_by_id_version(a.keyring,bm["key_id"],bm["key_version"],for_encrypt=False)
        dek=unwrap_dek(
            kek,bm["wrapped_dek_nonce_b64"],bm["wrapped_dek_b64"],
            wrap_aad(bm["backupset_id"],bm["framework_sha256"],bm["key_id"],bm["key_version"])
        )
        core=dict(bm)
        mn=core.pop("manifest_auth_nonce_b64")
        mt=core.pop("manifest_auth_tag_b64")
        verify_manifest(dek,core,mn,mt)
        ...
        for e in bm.get("files",[]):
            ...
            if len(cipher)!=int(e["ciphertext_size"]) or sha256(cipher)!=e["ciphertext_sha256"]:
                raise RuntimeError("encrypted payload ciphertext integrity mismatch: "+str(rel))
            aad=file_aad(
                bm["backupset_id"],str(e["path"]),bm["framework_sha256"],
                bm["key_id"],bm["key_version"],int(e["plaintext_size"])
            )
            plain=decrypt_file(dek,e["nonce_b64"],cipher,aad)
            if len(plain)!=int(e["plaintext_size"]):
                raise RuntimeError("encrypted payload plaintext size mismatch: "+str(rel))
        ...
        cp=subprocess.run(
            [sys.executable,str(SCORER),"--private-ledger-root",str(dest),"--output",str(out)],
        ...
        receipt={
          "version":"1.0","artifact_class":"JNU_PRIVATE_RESTORE_AUTHORIZATION_RECEIPT",
          "storage_scope":"PRIVATE_INTERNAL_ONLY","public_distribution_permitted":False,
          "restore_id":av["restore_id"],"backupset_id":bm["backupset_id"],
          "key_id":bm["key_id"],"key_version":bm["key_version"],
          "restore_status":"PASS","recovery_scan":"PASS","scorer_revalidation":"PASS",
          "authorization_consumed":True
        }
        write_replace_json(dest/"recovery"/"restore_authorization_receipt.json",receipt)
        mark_authorization_used(usage_receipt,av,bm)

    print(json.dumps({
      "status":"PRIVATE_ENCRYPTED_BACKUP_IMPORTED_AND_REVALIDATED",
      "backupset_id":bm["backupset_id"],"key_id":bm["key_id"],"key_version":bm["key_version"],
      "file_count":len(decoded),"restore_id":av["restore_id"],"authorization_consumed":True,
      "recovery_scan":"PASS","scorer_revalidation":"PASS",
      "key_material_printed":False,"raw_private_data_printed":False,"public_output_created":False
    },indent=2))
