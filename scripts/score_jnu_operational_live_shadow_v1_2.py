...
OUT=ROOT/"live_shadow"/"results"/"jnu_operational_live_shadow_current_v1_2.json"
...
def main():
    ...
    if args.forecast_dir.exists():
        for p in sorted(args.forecast_dir.glob("*.json")):
            ...
            if not isinstance(meta,dict): raise RuntimeError(f"forecast {fid} reference source metadata missing")
            if meta.get("exact_product") is not True or meta.get("continuous_contract") is not False: raise RuntimeError(f"forecast {fid} reference source product identity invalid")
            if meta.get("source_id")!="OSE": raise RuntimeError(f"forecast {fid} reference source_id invalid")
            import re as _re
            if not _re.fullmatch(r"NK225MC[A-Z][0-9]{4}",str(meta.get("symbol",""))): raise RuntimeError(f"forecast {fid} reference source contract invalid")
            if abs(float(meta.get("price"))-float(f.get("reference_price")))>1e-12: raise RuntimeError(f"forecast {fid} reference source price mismatch")
            if str(meta.get("source_timestamp"))!=str(f.get("reference_timestamp")): raise RuntimeError(f"forecast {fid} reference source timestamp mismatch")
            if meta.get("freshness_pass") is not True: raise RuntimeError(f"forecast {fid} reference source freshness flag invalid")
            mage=float(meta.get("freshness_age_seconds"))
            if mage<0 or mage>900: raise RuntimeError(f"forecast {fid} reference source age invalid")
