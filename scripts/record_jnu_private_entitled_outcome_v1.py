...
def main():
    ...
    outside(a.private_ledger_root,"private ledger root");outside(a.private_close_evidence,"private close evidence")
    ...
    f=load(fp)
    if f.get("artifact_class")!="JNU_PRIVATE_ENTITLED_FORECAST" or f.get("storage_scope")!="PRIVATE_INTERNAL_ONLY":raise RuntimeError("private forecast class/storage invalid")
    e=load(a.private_close_evidence)
    ...
    ref=float(f["reference_price"]);close=float(e["price"]);ret=close/ref-1.0
    bias=f["bias"]
    ...
    outdir=a.private_ledger_root.resolve()/"outcomes";outside(outdir,"private outcome directory");outdir.mkdir(parents=True,exist_ok=True)
    out=outdir/f"{a.forecast_id}.json"
    if out.exists():raise RuntimeError("private outcome already exists")
    rec={"version":"1.0","artifact_class":"JNU_PRIVATE_ENTITLED_OUTCOME","forecast_id":a.forecast_id,
      "storage_scope":"PRIVATE_INTERNAL_ONLY","public_distribution_permitted":False,
      "forecast_record_sha256":canonical_text_sha256(fp),"forecast_record_sha256_method":"SHA256_TEXT_EOL_CRLF_V1",
      "symbol":f["symbol"],"target_day_session_date":f["target_day_session_date"],
      "target_close_price":close,"target_close_timestamp":close_ts.isoformat(),
      "entitled_close_evidence":e,"entitled_close_validation":qv,"exact_product":True,
      "outcome_return":ret,"signed_outcome_return":signed,"directional_hit":hit,
      "immutable_record":True,"public_output_created":False}
